            ...
            if self.config.model.checkpoint_path is not None:
                ...
                if to_load_path != self.config.model.path:
                    ...
                    with FSDP.state_dict_type(
                        self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, cfg
                    ):
                        optimizer_state_dict = torch.load(
                            os.path.join(to_load_path, "optimizer.pt"),
                            weights_only=True,
                        )
                        ...
                        self.actor_optimizer.load_state_dict(flattened_osd)
                        self.actor_lr_scheduler.load_state_dict(
                            optimizer_state_dict["lr_scheduler"]
                        )
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        with FSDP.state_dict_type(
            ...
            optimizer_state_dict = FSDP.optim_state_dict(
                self.actor.actor_module, self.actor_optimizer
            )
            print("model dict size:", len(state_dict))
            print("optimizer dict size:", len(optimizer_state_dict["state"]))
        ...
        if self.rank == 0:
            ...
            print(
                f"Sharded state checkpoint saved to {local_path}; Time = {t1 - t0:.4f}\n"
            )
            ...
            t0 = time.perf_counter()
            torch.save(
                {"optimizer": optimizer_state_dict, "lr_scheduler": scheduler_state},
                os.path.join(local_path, "optimizer.pt"),
            )
            t1 = time.perf_counter()
            print(
